---
title: "Animation of Matthews Correlation Coefficient"
...
knit: (function(input_file, encoding) {
  ...
  rmarkdown::render(input_file,
    encoding=encoding,
    output_file=file.path(dirname(input_file), out_dir, 'AnimatedMCC.html'))})
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
metric.name <- "Matthews Correlation Coefficient"
metric.abbr <- "MCC"
```
...
These animated plots show how the contours of `r metric.name` (`r metric.abbr`) change with class balance, i.e., as the number of negative examples ($n$) and positive examples ($p$) vary in confusion matrices of fixed size ($N$). 

Each frame of this animation shows a two dimensional slice through the tetrahedral [confusion simplex](https://davidrlovell.github.io/Never-mind-the-metrics/ConfusionSimplex.html), a projection of the four dimensional confusion matrices of size $100$ into three dimensions. The animation shows slices sweeping from the edge of the simplex where $\mathrm{TP}=p, \mathrm{TN}=n$ through to the edge where $\mathrm{FN}=p, \mathrm{FP}=n$.
...
* As we show elsewhere, the `r metric.name` contours describe a series of concentric ellipses whose eccentricity depends on prevalence.
